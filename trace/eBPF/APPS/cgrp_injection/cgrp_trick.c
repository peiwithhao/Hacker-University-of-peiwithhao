...
int main(int argc, char **argv)
{
    ...
    bpf_map_update_elem(bpf_map__fd(skel->maps.filter_jump_table), &(uint32_t){0}, &prog_fd, BPF_ANY);
    // cpu 文件过滤
    prog_fd = bpf_program__fd(skel->progs.cgrp_ftype_filter_cpu);
    bpf_map_update_elem(bpf_map__fd(skel->maps.filter_jump_table), &(uint32_t){1}, &prog_fd, BPF_ANY);
    // rdma 文件过滤
    prog_fd = bpf_program__fd(skel->progs.cgrp_ftype_filter_rdma);
    bpf_map_update_elem(bpf_map__fd(skel->maps.filter_jump_table), &(uint32_t){2}, &prog_fd, BPF_ANY);
    // pids 文件过滤
    prog_fd = bpf_program__fd(skel->progs.cgrp_ftype_filter_pids);
    bpf_map_update_elem(bpf_map__fd(skel->maps.filter_jump_table), &(uint32_t){3}, &prog_fd, BPF_ANY);
    // misc 文件过滤
    prog_fd = bpf_program__fd(skel->progs.cgrp_ftype_filter_misc);
    bpf_map_update_elem(bpf_map__fd(skel->maps.filter_jump_table), &(uint32_t){4}, &prog_fd, BPF_ANY);

    /* err = cgrp_trick_bpf__attach(skel); */
    // NOTE: 这里注意因为需要使用bpf_tail_call 所以这里并不会全部attach,而是选择部分attach
    // 加载过滤文件主程序
