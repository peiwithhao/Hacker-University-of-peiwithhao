...
static __always_inline int bpf_strnstr(const char *haystack, int haystack_size, const char *needle, int needle_size) {
/* static int bpf_strnstr(const char *haystack, int haystack_size, const char *needle, int needle_size) { */
    int i, j;
...
int BPF_PROG(cgrp_ftype_filter_memory, int dfd, const char *filename, struct open_how *how, int ret){
    ...
    if(!bpf_strnstr(haystack, sizeof(haystack), memory_current_substr, sizeof(memory_current_substr))){
        pid_fd_value = TYPE_MEMORY_CURRENT;
        /* bpf_printk("memory reading file, fd: %d, %s, ", ret, filename); */
    }
    else if(!bpf_strnstr(haystack, sizeof(haystack), memory_max_substr, sizeof(memory_max_substr)))
        pid_fd_value = TYPE_MEMORY_MAX;
    else if(!bpf_strnstr(haystack, sizeof(haystack), memory_swap_current_substr, sizeof(memory_swap_current_substr)))
        pid_fd_value = TYPE_MEMORY_SWAP_CURRENT;
    else if(!bpf_strnstr(haystack, sizeof(haystack), memory_swap_max_substr, sizeof(memory_swap_max_substr)))
...
SEC("fexit/do_sys_openat2")
int BPF_PROG(cgrp_ftype_filter_cpu, int dfd, const char *filename, struct open_how *how, int ret){
    ...
    if(!bpf_strnstr(haystack, sizeof(haystack), cpu_weight_substr, sizeof(cpu_weight_substr)))
        pid_fd_value = TYPE_CPU_WEIGHT;
    else if(!bpf_strnstr(haystack, sizeof(haystack), cpu_max_substr, sizeof(cpu_max_substr)))
...
SEC("fexit/do_sys_openat2")
int BPF_PROG(cgrp_ftype_filter_rdma, int dfd, const char *filename, struct open_how *how, int ret){
    ...
    if(!bpf_strnstr(haystack, sizeof(haystack), rdma_max_substr, sizeof(rdma_max_substr)))
        pid_fd_value = TYPE_RDMA_MAX;
    else if(!bpf_strnstr(haystack, sizeof(haystack), rdma_current_substr, sizeof(rdma_current_substr)))
...
SEC("fexit/do_sys_openat2")
int BPF_PROG(cgrp_ftype_filter_pids, int dfd, const char *filename, struct open_how *how, int ret){
    ...
    if(!bpf_strnstr(haystack, sizeof(haystack), pids_max_substr, sizeof(pids_max_substr)))
        pid_fd_value = TYPE_PIDS_MAX;
    else if(!bpf_strnstr(haystack, sizeof(haystack), pids_current_substr, sizeof(pids_current_substr)))
...
SEC("fexit/do_sys_openat2")
int BPF_PROG(cgrp_ftype_filter_misc, int dfd, const char *filename, struct open_how *how, int ret){
    ...
    if(!bpf_strnstr(haystack, sizeof(haystack), misc_current_substr, sizeof(misc_current_substr)))
...
SEC("fexit/do_sys_openat2")
int BPF_PROG(cgrp_ftype_filter, int dfd, const char *filename, struct open_how *how, int ret){
    u32 key = 0;
...
SEC("fexit/ksys_read")
int BPF_PROG(cgrp_read_hijack, unsigned int fd, const char * buf, size_t count, ssize_t ret){
    u32 key = 0;
    u32 *filter_pid = bpf_map_lookup_elem(&filter, &key);
    u64 pid_tgid = bpf_get_current_pid_tgid();
    if (filter_pid && *filter_pid != 0 && (*filter_pid != (pid_tgid >> 32))){
    ...
    case TYPE_MEMORY_CURRENT:
        ...
        /* bpf_printk("reading fd: %d [%ld, %s]",fd,  memory_current_hijack(dst, sizeof(dst)), dst); */
        break;
